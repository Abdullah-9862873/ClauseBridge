...
export default function FirmSettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/v1/firms/me`, {
      ...
      .catch(() => {
        showToast({ message: 'Failed to load firm details.', type: 'error' });
      })
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      showToast({ message: 'Firm name cannot be empty.', type: 'error' });
      return;
    }
    setSaving(true);
    try {
      ...
      setFirm(data);
      showToast({ message: 'Firm name updated.', type: 'success' });
    } catch {
      showToast({ message: 'Failed to update firm name.', type: 'error' });
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="main-content">
        <div className="topbar">
          <div className="breadcrumb">
            ...
            <span className="seg-current">Firm settings</span>
          </div>
        ...
        <div className="screen">
          <div className="page-head">
            <div>
              <h2>Firm settings</h2>
              <p className="sub">Manage your firm details</p>
            </div>
          </div>

          {loading ? (
            <div style={{ padding: '40px 20px', color: 'var(--ink-50)', fontSize: '14px' }}>Loading...</div>
          ) : (
            <div className="card" style={{ maxWidth: '480px' }}>
              ...
              <form onSubmit={handleSave} style={{ padding: '16px 20px' }}>
                ...
                <div style={{ marginBottom: '12px' }}>
                  <div style={{ fontSize: '12px', color: 'var(--ink-50)', marginBottom: '4px' }}>Plan</div>
                  <div style={{ fontSize: '14px', textTransform: 'capitalize' }}>{firm?.plan_tier || 'free'}</div>
                </div>
                <button type="submit" className="btn btn-primary" disabled={saving} style={{ width: 'auto', marginTop: '4px' }}>
